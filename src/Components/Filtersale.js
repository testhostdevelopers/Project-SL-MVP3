...
export default function Filtersale({ }) {
  ...
  useEffect(() => {
    let checkedV = document.querySelectorAll(".filtersale-check input[type='checkbox']");
    let arr = [];
    ...
    <li>
      ...
      <div className="ant-select ant-select-single ant-select-show-arrow" onClick={handleToggle}>
        <div className="ant-select-selector">
          <span className="ant-select-selection-item">{isChecked ? checkedValues : 'Sale type'}</span>
        </div>
      ...
      <div className={`app ${isActive ? "custom-filter show-filter" : "custom-filter"}`}>
        <ul className="filtersale-check">
          ...
          <li>
            <input type="checkbox" id="Openforoffers" value="Open for offers" />
            <label for="Openforoffers">
              Open for offers
            </label>
          </li>
        </ul>
